...
const processDiagrams = {
  'grafon-supply-chain': {
    definition: `flowchart LR
      A[Complex process] --> B[Flow-first structure]
      B --> C[Interactive simulation]
      C --> D[Clearer decisions]`,
    summary: 'The design translates a dense supply-chain problem into a visual flow that helps teams explore scenarios and understand outcomes.',
  },
  'tradedungeon-uiux': {
    definition: `flowchart LR
      A[Trust gap] --> B[Visible security cues]
      B --> C[Clear trade states]
      C --> D[Confident action]`,
    summary: 'The design reduces hesitation by placing trust signals and transaction status close to the decisions users need to make.',
  },
  'dataflower-baas': {
    definition: `flowchart LR
      A[Complex setup] --> B[Guided onboarding]
      B --> C[Clear API steps]
      C --> D[First successful call]`,
    summary: 'The design turns backend setup into a guided sequence so developers can understand the next action and reach first success faster.',
  },
  'powerion-energy': {
    definition: `flowchart LR
      A[Data overload] --> B[Priority signals]
      B --> C[Actionable dashboard]
      C --> D[Operational response]`,
    summary: 'The design moves energy data from passive reporting toward prioritized signals that help operators decide what needs attention.',
  },
}

Object.values(caseStudiesBySlug).forEach((study) => {
  study.page.diagram = processDiagrams[study.slug]
  study.sections.forEach((section) => {
    if (section.body?.length || section.summary) return
    const narratives = {
      audience: 'The interface decisions in this case study start with the people who need to use the experience, what they are trying to accomplish, and where confidence can break down.',
      role: 'My contribution connected UX structure, interface craft, visual consistency, and delivery support so the work could move from design intent into a usable product.',
      approach: 'This approach turns the brief into a sequence of design decisions: understand the problem, reduce friction, establish a system, and make the next action clear.',
      outcomes: 'These outcomes describe the practical value of the design work: clearer behavior, stronger consistency, and a more useful foundation for future product decisions.',
    }
    if (narratives[section.id]) section.body = [narratives[section.id]]
  })
...
export function getCaseStudyPagination(currentSlug) {
  const currentIndex = caseStudySlugs.indexOf(currentSlug)
  if (currentIndex < 0) return {}

  const previous = caseStudySlugs[currentIndex - 1]
  const next = caseStudySlugs[currentIndex + 1]
  return {
    previous: previous ? toCaseStudyCard(previous) : undefined,
    next: next ? toCaseStudyCard(next) : undefined,
  }
}
